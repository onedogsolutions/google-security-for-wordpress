...
JS;
			...
			<td>
				<?php if ($enabled): ?>
					<p><span class="dashicons dashicons-yes" style="color:#46b450"></span>
						<?php esc_html_e('Two-factor authentication is enabled for this user.', 'google-security-for-wordpress'); ?>
					</p>
					<?php if (current_user_can('edit_user', $user->ID)): ?>
						<label><input type="checkbox" name="gswp_2fa_admin_disable" value="1" />
							<?php esc_html_e('Disable two-factor authentication for this user', 'google-security-for-wordpress'); ?></label>
						<p class="description">
							<?php esc_html_e('Use this to recover access for a user who has lost their authenticator and backup codes.', 'google-security-for-wordpress'); ?>
						</p>
					<?php endif; ?>
				<?php else: ?>
					<p><?php esc_html_e('Two-factor authentication is not enabled for this user.', 'google-security-for-wordpress'); ?>
					</p>
				<?php endif; ?>
			</td>
		</tr>
	...
	private function render_manage_rows($user)
	{
		...
		<tr>
			...
			<td>
				<p><span class="dashicons dashicons-yes" style="color:#46b450"></span>
					<?php esc_html_e('Two-factor authentication is active on your account.', 'google-security-for-wordpress'); ?>
				</p>
				<p class="description">
					<?php
				...
				<p><label><input type="checkbox" name="gswp_2fa_regen" value="1" />
						<?php esc_html_e('Generate a new set of backup codes', 'google-security-for-wordpress'); ?></label>
				</p>
				<p><label><input type="checkbox" name="gswp_2fa_disable" value="1" />
						<?php esc_html_e('Disable two-factor authentication', 'google-security-for-wordpress'); ?></label></p>
			</td>
		</tr>
		<?php
	}
	...
	private function render_setup_rows($user)
	{
		...
		<tr>
			...
			<td>
				<p><?php esc_html_e('Scan this QR code with Google Authenticator (or any compatible app), then enter the 6-digit code it shows to finish.', 'google-security-for-wordpress'); ?>
				</p>
				<div id="gswp-2fa-qr" data-otpauth="<?php echo esc_attr($uri); ?>"
					style="width:160px;height:160px;background:#fff;padding:8px;border:1px solid #dcdcde;"></div>
				<p style="margin-top:12px;">
					<?php esc_html_e("Can't scan it? Enter this setup key manually:", 'google-security-for-wordpress'); ?><br />
					<code
						style="font-size:14px;letter-spacing:1px;"><?php echo esc_html($this->format_secret($secret)); ?></code>
				</p>
			</td>
		</tr>
		<tr>
			<th scope="row"><label
					for="gswp_2fa_setup_code"><?php esc_html_e('Verification code', 'google-security-for-wordpress'); ?></label>
			</th>
			<td>
				<input type="text" name="gswp_2fa_setup_code" id="gswp_2fa_setup_code" class="regular-text" inputmode="numeric"
					autocomplete="off" />
				<p class="description">
					<?php esc_html_e('Enter the current 6-digit code and save your profile to enable two-factor authentication.', 'google-security-for-wordpress'); ?>
				</p>
	...
	private function maybe_render_notices($user_id)
	{
		...
		if (isset($_GET['gswp_2fa_required']) && get_current_user_id() === $user_id) {
			echo '<div class="notice notice-error inline"><p>' . esc_html__('Your account requires two-factor authentication. Set it up below to continue. You will not be able to login again until you complete this step', 'google-security-for-wordpress') . '</p></div>';
		}
