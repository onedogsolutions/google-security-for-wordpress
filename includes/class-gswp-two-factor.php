...
class GSWP_Two_Factor {
	...
	/** Cookie holding a trusted-browser token, formatted "user_id|token". */
	const COOKIE_TRUSTED = 'gswp_2fa_trusted';
	...
	/** Maximum remembered browsers kept per user. */
	const TRUSTED_MAX = 10;
	...
	public function __construct() {
		...
		// Revoke remembered browsers when the password is reset, so a stolen or
		// shared credential cannot ride an old trusted-device cookie.
		add_action( 'after_password_reset', array( $this, 'forget_all_devices' ), 10, 1 );
	}

	/**
	 * Whether users may mark a browser as trusted to skip the second factor.
	 *
	 * @return bool
	 */
	...
	public function enforce_second_factor( $user, $username, $password ) {
		...
		// A browser the user previously chose to trust skips the second factor
		// for the trust window — unless Account Defender flagged this login as
		// suspicious, in which case the challenge is always required.
	...
	public function ajax_verify() {
		...
		// Optionally trust this browser so it can skip the second factor for the
		// trust window. Set before the JSON body is sent so the cookie sticks.
		$remember_device = isset( $_POST['remember_device'] ) && '1' === $_POST['remember_device']; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( $remember_device && self::remember_enabled() ) {
	...
	public function enqueue_modal_assets() {
		...
		wp_localize_script(
			...
			array(
				...
				'home'            => home_url( '/' ),
				'rememberEnabled' => self::remember_enabled(),
				'i18n'            => array(
					...
					'remember' => __( 'Remember this browser for 30 days', 'google-security-for-wordpress' ),
				),
			)
		);
	}
	...
	/**
	 * Whether the current request carries a valid trusted-browser cookie.
	 *
	 * The cookie holds "user_id|token"; only the token's HMAC is stored, so the
	 * plaintext cookie can be matched but never derived from the database. The
	 * cookie is bound to the user it was issued to, so it can only waive the
	 * second factor for that same account.
	 *
	 * @param int $user_id User being authenticated.
	 * @return bool
	 */
	private function is_trusted_device( $user_id ) {
		...
		$raw = isset( $_COOKIE[ self::COOKIE_TRUSTED ] ) ? sanitize_text_field( wp_unslash( $_COOKIE[ self::COOKIE_TRUSTED ] ) ) : '';
		if ( '' === $raw || false === strpos( $raw, '|' ) ) {
			return false;
		}

		list( $cookie_uid, $token ) = explode( '|', $raw, 2 );
		if ( (int) $cookie_uid !== (int) $user_id || '' === $token ) {
			return false;
		}
		...
		foreach ( $devices as $index => $device ) {
			// Drop malformed or expired records as we go.
			if ( empty( $device['hash'] ) || empty( $device['expires'] ) || $device['expires'] < $now ) {
				unset( $devices[ $index ] );
				$changed = true;
				continue;
			}
		...
		if ( $changed ) {
			update_user_meta( $user_id, self::META_TRUSTED, array_values( $devices ) );
		}
	...
	/**
	 * Trust the current browser: store a hashed token and set the cookie.
	 *
	 * @param int $user_id User to trust this browser for.
	 */
	private function remember_device( $user_id ) {
		...
		// Prune expired records before adding the new one.
		foreach ( $devices as $index => $device ) {
			if ( empty( $device['expires'] ) || $device['expires'] < $now ) {
				unset( $devices[ $index ] );
			}
		}
		...
		$devices[] = array(
			...
			'created' => $now,
			'ua'      => substr( hash( 'sha256', isset( $_SERVER['HTTP_USER_AGENT'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ) ) : '' ), 0, 16 ),
		);

		// Keep only the most recent trusted browsers to bound the meta size.
		$devices = array_slice( array_values( $devices ), -self::TRUSTED_MAX );

		update_user_meta( $user_id, self::META_TRUSTED, $devices );

		$this->set_trusted_cookie( $user_id . '|' . $token, $now + self::TRUSTED_TTL );
	}

	/**
	 * Number of a user's currently valid (unexpired) trusted browsers.
	 *
	 * @param int $user_id User ID.
	 * @return int
	 */
	public static function trusted_device_count( $user_id ) {
		$devices = get_user_meta( $user_id, self::META_TRUSTED, true );
		if ( ! is_array( $devices ) ) {
			return 0;
		}
	...
	/**
	 * Forget every remembered browser for a user (revocation).
	 *
	 * @param WP_User|int $user User object (from after_password_reset) or ID.
	 */
	public function forget_all_devices( $user ) {
	...
	/**
	 * HMAC a trusted-device token with the site's auth salt for storage/lookup.
	 *
	...
	/**
	 * Emit the trusted-browser cookie (HTTP-only, Secure, SameSite=Lax).
	 *
	 * @param string $value   Cookie value ("user_id|token").
	 * @param int    $expires Unix expiry timestamp.
	 */
	private function set_trusted_cookie( $value, $expires ) {
		if ( headers_sent() ) {
			return;
		}
		...
		$_COOKIE[ self::COOKIE_TRUSTED ] = $value;
	}

	/**
	 * Expire the trusted-browser cookie on the current browser.
	 */
	private function clear_trusted_cookie() {
		if ( ! headers_sent() ) {
			setcookie(
				self::COOKIE_TRUSTED,
				'',
		...
		unset( $_COOKIE[ self::COOKIE_TRUSTED ] );
	}

	/**
	 * Whether Account Defender has forced a 2FA step-up for this request.
	 *
	 * @return bool
	 */
	private function step_up_forced() {
		return class_exists( 'GSWP_Account_Defender' )
			&& GSWP_Account_Defender::is_active()
			&& GSWP_Account_Defender::should_force_2fa();
	}
...
JS;
	...
	private function render_manage_rows( $user ) {
		...
		<tr>
			...
			<td>
				...
				<?php if ( self::remember_enabled() ) : ?>
					<?php $trusted = self::trusted_device_count( $user->ID ); ?>
					<p class="description" style="margin-top:12px;">
						<?php
						/* translators: %d: number of remembered browsers. */
						echo esc_html( sprintf( _n( '%d browser is currently remembered and can skip the code.', '%d browsers are currently remembered and can skip the code.', $trusted, 'google-security-for-wordpress' ), $trusted ) );
						?>
					</p>
					<?php if ( $trusted > 0 ) : ?>
						<p><label><input type="checkbox" name="gswp_2fa_forget_devices" value="1" /> <?php esc_html_e( 'Forget all remembered browsers (require the code again everywhere)', 'google-security-for-wordpress' ); ?></label></p>
					<?php endif; ?>
				<?php endif; ?>
	...
	public function save_profile( $user_id ) {
		...
		// Forget remembered browsers (require the code again everywhere).
		if ( ! empty( $_POST['gswp_2fa_forget_devices'] ) && self::user_has_2fa( $user_id ) ) {
			delete_user_meta( $user_id, self::META_TRUSTED );
			$this->clear_trusted_cookie();
		}
