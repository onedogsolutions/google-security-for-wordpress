...
echo "        \$t = isset( \$d['gswp_ff_token'] ) ? (string) \$d['gswp_ff_token'] : null;\n";
echo "        error_log( 'GSWP POST keys: ' . implode( ',', array_keys( \$_POST ) ) );\n";
echo "        error_log( 'GSWP formData keys: ' . implode( ',', array_keys( (array) \$d ) ) );\n";
echo "        error_log( 'GSWP token: ' . ( null === \$t\n";
echo "            ? 'ABSENT (key not in formData at all)'\n";
echo "            : ( '' === \$t\n";
echo "                ? 'PRESENT BUT EMPTY (field rendered, JS never filled it)'\n";
echo "                : 'POPULATED (len ' . strlen( \$t ) . ')' ) ) );\n";
echo "        return \$e;\n";
echo "    }, 1, 4 );\n\n";
echo "  *** THREE STATES, NOT TWO. An earlier version of this snippet used\n";
echo "  isset() alone and printed only yes/no. isset() RETURNS TRUE FOR AN\n";
echo "  EMPTY STRING, so a rendered-but-unfilled field printed 'yes' —\n";
echo "  indistinguishable from a real token. submitted_token() uses empty(),\n";
echo "  not isset(), so an empty field resolves to '' and takes the\n";
echo "  missing-token branch (a REJECTION on a strict form). Any test that\n";
echo "  turns on whether a token was really produced needs all three states,\n";
echo "  and the two-state version silently invalidated a charge-timing run on\n";
echo "  the reporting site. ***\n";
echo "\n";
echo "  Expected on 2.23.2, with the provider ON:\n";
echo "    GSWP formData keys : ...,gswp_ff_token,... (our field survived)\n";
echo "    GSWP token         : POPULATED (len ...)\n";
echo "\n";
echo "  ABSENT means the whitelist registration checked above did not take\n";
echo "  effect on this request — report the whitelist table alongside it. The\n";
echo "  submission is still scored, because transport 2 reads the raw envelope\n";
echo "  as a backstop; a finding to report, not an outage.\n";
echo "\n";
echo "  PRESENT BUT EMPTY means our field rendered but the browser never\n";
echo "  produced a token. On a strict form that is a REJECTION, and it is the\n";
echo "  state chunk 27 deliberately induces to test charge timing.\n";
echo "\n";
echo "  STEP 2 — confirm a rejection is VISIBLE. On a form you do not mind\n";
echo "  breaking temporarily, force a rejection and confirm the message\n";
echo "  actually appears on screen — not just that the request returns an\n";
echo "  error.\n";
echo "\n";
echo "  Force it by removing our field in the DevTools console immediately\n";
echo "  before clicking submit:\n\n";
echo "      document.querySelector('input[name=\"gswp_ff_token\"]').remove();\n\n";
echo "  Do NOT do this by blocking www.google.com/recaptcha in the network\n";
echo "  tab. An earlier revision of this chunk said to, and on the reporting\n";
echo "  site it did not reject anything: blocking the challenge bundle does\n";
echo "  not stop Enterprise minting a score-only token, so the submission was\n";
echo "  scored, admitted and charged while the test appeared to have run. See\n";
echo "  chunk 27 section C.\n";
echo "\n";
echo "  Do this once with a form set to 'inline' placement and once with\n";
